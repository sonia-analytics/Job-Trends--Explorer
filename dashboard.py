...
# --- Data Loading ---
uploaded_file = st.file_uploader("Upload clean_job_data.csv or .xlsx", type=["csv", "xlsx"])
if uploaded_file:
    ...
    try:
        df = pd.read_csv("clean_job_data.csv")  # Changed from .xlsx to .csv
    except:
        st.error("Please upload or place clean_job_data.csv in the app folder.")
        st.stop()
